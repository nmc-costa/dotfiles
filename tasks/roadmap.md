...
flowchart LR
    ...
    subgraph proj_dotfiles["dotfiles"]
        ...
        dotfiles-tsk-systemd-units["dotfiles-tsk-systemd-units<br/>ensure-tsk-sweep.sh + systemd units, following the existing ensure-*.sh convention"]:::done
        ...
        dotfiles-tsk-harness-provider-model-index["dotfiles-tsk-harness-provider-model-index<br/>Build a validated harness x provider x model orchestration index"]:::done
        ...
        dotfiles-tsk-agent-actor-safety["dotfiles-tsk-agent-actor-safety<br/>[HIGH PRIORITY] Agents must sign tasks/ events as actor-kind=agent for their own writes, and must not run unguarded git ops on the shared tasks/events.jsonl working tree"]:::done
        ...
        dotfiles-tsk-orchestra-smoke["dotfiles-tsk-orchestra-smoke<br/>Smoke test real do tasks/orchestra.py --launch (plan §Verification 3): 1 agente claude + 1 agente copilot"]:::backlog
        dotfiles-tsk-worktree-model-suffix["dotfiles-tsk-worktree-model-suffix<br/>tasks/worktree.py v2: optional [-<model>] suffix in path/branch when a harness has >1 agent on the same card"]:::backlog
        dotfiles-tsk-chronicle-skill-layer["dotfiles-tsk-chronicle-skill-layer<br/>Chronicle-like skill layer: mine interactions into skills/instructions, resume-after-timeout, PR-finish, skill chaining + chat autocomplete"]:::done
        dotfiles-tsk-dispatch-quota-check["dotfiles-tsk-dispatch-quota-check<br/>dispatch.py: post-launch liveness probe + record provider quota status/reset time"]:::backlog
        dotfiles-tsk-chronicle-d4-d6["dotfiles-tsk-chronicle-d4-d6<br/>Chronicle slice D4-D6: /pr-finish, /chronicle miner, footer+questionario conventions"]:::done
        dotfiles-tsk-chronicle-d7["dotfiles-tsk-chronicle-d7<br/>Chronicle D7: opencode custom commands + tui.prompt.append chain bridge"]:::validation
        dotfiles-tsk-chronicle-daily-improve["dotfiles-tsk-chronicle-daily-improve<br/>Chronicle daily background improve: timer, headless opencode run, gated auto-merge PRs"]:::review
<<<<<<< HEAD
        dotfiles-tsk-radar-opencode-backend["dotfiles-tsk-radar-opencode-backend<br/>Radar family: commit skills+automation+units, omarchy-radar backend routing (opencode default)"]:::in_progress
=======
>>>>>>> 0d97f4fa7bd83eaa3f3232b5082e639546fbf350
    end
...
timeline
    ...
    2026-09-25 : dotfiles-tsk-github-to-agents : dotfiles-tsk-chronicle-skill-layer : dotfiles-tsk-agent-actor-safety : dotfiles-tsk-harness-provider-model-index : dotfiles-tsk-systemd-units : dotfiles-tsk-chronicle-d4-d6
```
