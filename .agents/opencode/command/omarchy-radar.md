---
description: Omarchy community digest — run the radar and get a compact brief summary in chat.
---

Use the `omarchy-radar` skill (loaded from `~/.agents/skills/omarchy-radar/SKILL.md`).
...
1. If today's brief already exists, read it directly and skip to step 4:
   `git -C ~/dotfiles show radar/omarchy-radar/$(date +%F):briefs/omarchy-radar-$(date +%F).md`
2. Otherwise run `~/dotfiles/.agents/skills/omarchy-radar/scripts/run.sh --prepare`.
...
3. Read the prompt file it prints — **you are the agent step**: score the inbox
   ...
   `~/dotfiles/.agents/skills/omarchy-radar/scripts/run.sh --finalize`.
4. End your reply with a keyword-compact summary of the brief in chat: what's
   ...
   `radar/omarchy-radar/<date>` branch for the human merge.
