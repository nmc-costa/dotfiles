...
# radar-common/lib.sh. Implements the plan's "The agent step" sequence in
# three modes:
#
#   run.sh                FULL AUTO (timer mode, used unattended by
#                         omarchy-radar.timer): prepare → nested sandboxed
...
prepare_step() {
  ...
  # Step 0 (prerequisite, not one of the agent-step steps): collect is
  # idempotent and safe to call every prepare -- a same-day re-run finds
  # nothing new rather than duplicating today's inbox (see collect.sh's own
  # header comment).
