...
- [x] **New skill `harness-orchestra`** (2026-09-25) — fans a `tasks/` card out across several harness+provider+model agents at once (e.g. 1 Claude Opus + 1 Copilot GPT-5.5), each in its own `agent-deck`-managed worktree/branch, closing via `.agents/skills/handoff/handoff.py` + `events.jsonl` — never IPC. Thin `tasks/orchestra.py` (launch/status/collect) does the real work; adopts `agent-deck` as the L2 execution engine (`tasks/README.md`, decided 2026-09-18) rather than building a worktree engine of its own. Design: `tasks/plans/cross-harness-orchestra.md`. Symlinked in `.claude/skills/` and `.github/skills/`; run `./sync.sh` to install.
