...
| Add/edit an always-loaded core block (`OUTPUT-FRAME:CORE`, later `AUTONOMY:CORE`) | Edit the canonical `.agents/instructions/workspace-config/<name>.instructions.md`, then the marker block **identically** in every file listed in `scripts/core_blocks.manifest` (today 7: `AGENTS.md`, `GEMINI.md`, `.agents/rules/output-frame.md`, `.claude/CLAUDE.md`, `.gemini/GEMINI.md`, `.codex/AGENTS.md`, `.copilot/copilot-instructions.md`). `scripts/check_core_blocks.sh` (run by `validate_dotfiles.sh` and CI) fails on drift. |
| Read/review a radar's proposed changes, or add a new radar | `briefs/` — root folder, `.gitkeep` only on `main`; a real `briefs/<radar-name>-<date>.md` (plus optional `*.proposals/`) only ever exists on a `radar/<radar-name>/<date>` git-worktree branch, never on `main`. See `.agents/skills/researcher-radar/SKILL.md` for the family, `.agents/skills/{omarchy-radar,harness-radar}/SKILL.md` for each member, and `.agents/automation/radar-common/lib.sh` for the shared plumbing they both call into. |
...
- [ ] **Spot-check the Output Frame on each harness** (Claude Code, Copilot CLI, Gemini CLI/Antigravity, Codex) with the 4 S/M/L/exempt prompts listed in the Output Frame implementation PR — plan `tasks/plans/agent-output-tldr-format.md` §6 S5. S6 (a Claude `Stop` hook) only if Claude Code compliance is poor after 2 weeks of real use.
- [x] **New skill family `researcher-radar`** (2026-09-25) — headless daily-digest radars: `researcher-radar` (meta-skill, `.agents/skills/researcher-radar/`) documents the family and points at two member radars, `omarchy-radar` (Omarchy Linux/Quattro community) and `harness-radar` (AI coding-agent/harness ecosystem + SWE-bench/SWE-bench Verified/Terminal-Bench/LiveCodeBench benchmark leaderboards) — each a standalone `.agents/skills/<name>/` (`SKILL.md`/`README.md`/`sources.md`/`ranking.md`/`security.md`/`scripts/`). Both share `.agents/automation/radar-common/lib.sh` (state helpers, git-worktree isolation, the no-Bash `claude -p` wrapper, secret-scan, `flock`, notify) written once instead of duplicated. Two new systemd `--user` automations, `systemd/omarchy-radar/omarchy-radar.{service,timer}` (08:00) and `systemd/harness-radar/harness-radar.{service,timer}` (08:20) — installed via each radar's own `scripts/install_timer.sh`, never auto-enabled. Every radar only ever proposes (never applies) on a disposable `radar/<name>/<date>` branch for human review. New durable fact ledgers seeded at `docs/radar-knowledge/{omarchy,harness}.md`. Run `./sync.sh` to install the three skills.
