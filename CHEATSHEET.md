...
| Add/edit an always-loaded core block (`OUTPUT-FRAME:CORE`, later `AUTONOMY:CORE`) | Edit the canonical `.agents/instructions/workspace-config/<name>.instructions.md`, then the marker block **identically** in all 8 entry files (`AGENTS.md`, `GEMINI.md`, `.github/copilot-instructions.md`, `.agents/rules/output-frame.md`, `.claude/CLAUDE.md`, `.gemini/GEMINI.md`, `.codex/AGENTS.md`, `.copilot/copilot-instructions.md`). `scripts/check_core_blocks.sh` (run by `validate_dotfiles.sh` and CI) fails on drift. |
