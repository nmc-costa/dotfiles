...
| Non-Claude harnesses (Copilot, Gemini, Antigravity) auto-reading `tasks/`+`CHEATSHEET.md` on startup | **Verified via a real diagnostic prompt, 2026-09-16 (round 2)**: fixed for Copilot Chat (VS Code extension) and, per Antigravity's own self-reported discovery mechanism, plausibly fixed for Antigravity too (added redundantly to `AGENTS.md`, `GEMINI.md`, and the new `.agents/rules/session-startup.md` — Antigravity confirmed it scans all three paths, but hadn't picked any up yet in the test session, so re-verify). **Copilot CLI is a separate product from Copilot Chat and has no automatic file-reading mechanism at all** — nothing on the repo side fixes that, use the manual kickoff prompt. See "⚠️ Known Gaps" below |
...
- **Sync direction, resolved 2026-09-17**: `sync.sh` is repo→system (dotfiles is the source of truth), but as of the rewrite that day it's a per-file, manifest-based reconciliation, not a blind mirror — a file that only exists at the destination (e.g. written there by a live-installed tool) is never deleted, and `./sync.sh --pull` is the explicit system→repo direction for exactly that case. `~/.claude`, `~/.agents` are populated by `sync.sh` (a real copy, not a symlink, by design — see `.agents/harnesses/PROVIDERS.md` for why `providers/` specifically stays a copy too); `~/.vscode`/`~/.dtx-providers` ARE symlinks, set up by `setup.sh`'s `setup_agent_symlinks`. Don't assume any of `~/.claude`/`~/.agents`/`~/.vscode`/`~/.dtx-providers` without checking `ls -la` on the actual machine — the symlink-vs-copy split is real and intentional, but easy to misremember.
- **`.vscode/settings.json` contains a real API key** for a custom endpoint, and so does `~/.dtx-providers/secrets.env` (same key, consumed by `.agents/providers/`'s harness adapters). Both are migrated to chezmoi+age (done, see `docs/SECRETS.md`) — the plaintext value only ever exists in the gitignored, locally-decrypted files, never in anything tracked by git.
